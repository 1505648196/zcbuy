    ...
    function () {
        ...
        function getMeasurements(){
            $.get("getMeasurements",function (res) {
                ...
                $.each(dataPlus,function (index,item) {
                    var b=false;
                    //判断是否含有这个权限id  有就true
                    if (item== $("#measurementplus").val()){
                        b = true;
                    }
                    if(b){
                        htmltwo+="<option value='"+item+"' selected>"+item+"</option>";
                        b=false;
                    }else {
                        htmltwo+="<option value='"+item+"'>"+item+"</option>";
                    }
                });
                $("#measurement").html(htmltwo);
                form.render();
            });
        }
        ...
        form.on('submit(sub)',function (data) {
           ...
           var name = $("#name").val();
            ...
            $.post("updateGoods",
                //status状态没写  后台未定义好状态
                {"id":id,"name":name,"price":parseInt(price*100),"status":statusId,"goodsTypeId":goodsTypeId,"userId":userId,"measurement":measurement},//可以试试ES6 7 8 的语法
                function (res) {
                    if(res.result){
                        layer.msg(res.msg, {
                            time: 1000
